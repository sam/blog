...
object Posts extends Secured {
  ...
  def edit(id: String) = withAuth { username =>
    implicit request =>
      Async {
        for {
  ...
  def update(id: String) = withAuth { username =>
    implicit request =>
      Async {
        postForm.bindFromRequest.fold(
          errors => Category.titles map (categories => BadRequest(views.html.admin.Posts.edit(errors)(categories))),
  ...
  def delete(id: String) = withAuth { username =>
    implicit request =>
      Async {
        for ((_, post) <- Post.delete(id)) yield {
          Cache.set("modifiedAt", new Date().getTime)
  ...
  val postForm = Form[Post](
    mapping(
      ...
      "body" -> optional(text),
      "categories" -> list(text))((title, slug, publishedAt, body, categories) => Post(None, title, slug, publishedAt, body, Some(categories)))((post: Post) => Some((post.title, post.slug, post.publishedAt, post.body, post.categories.getOrElse(Nil).toList))))
}